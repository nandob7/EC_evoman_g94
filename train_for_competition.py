...
custom_fitness = False
random_start = True
path = f"runs/competition"

# Train with a single set of enemies or a predefined environment
for run in range(1):
    experiment_name = f'test_{run + 1}_100pop_1000gen_all_enemy'
    directory = os.path.join(path, f"run_{run + 1}")
    ...
    for generation in range(number_of_gen):
        ...
        parents = parent_selection(sorted_population_with_fitness, num_elite=params_ea["num_elite"], k=params_ea["k_members"])
